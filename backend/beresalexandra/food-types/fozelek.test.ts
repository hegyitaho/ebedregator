...
import * as wasd from '../../../test/test-utils'
import {fozelekSelectorsForTheWeek, fozelekDishes} from './fozelek'

test.before(t => {
  console.log(wasd)
  t.context['$'] = wasd.loadBeresAlexandraTestDouble()
})
test('fozelek selectors', t => {
  t.deepEqual(fozelekSelectorsForTheWeek(), [
...
test('fozeklek raw text', t => {
  t.deepEqual(fozelekDishes(t.context['$']), [
    [
